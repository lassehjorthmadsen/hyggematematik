...
$\frac{km}{t}$ er *enheden* for hastighed. Tricket er, at enhederne opfører sig som ethvert andet regnestyke. Hvis vi for eksempel havde haft:

$$\frac{8\ a}{2\ b} = 4 \frac{a}{b}$$
Vi kan omskrive det lidt mere, hvis det er svært umiddelbart at se, hvorfor regnestykket ovenfor er rigtigt:

$$\frac{8\ a}{2\ b} = \frac{8}{2} \cdot \frac{a}{b} = 4 \cdot \frac{a}{b}$$

Tricket er som sagt, at enheder opfører sig ligesom variablerne $a$ og $b$ i stykket herover.

Hvis vi i løbeeksemplet ikke ville udregne løbehastigheden mål i kilometer i timen, men i stedet for vide, hvor mange timer det tager at løbe en kilometer, ville regnestykket bare være den omvendte brøk:

$$\frac{1\ t}{10\ km} = 0,1 \frac{t}{km}$$
Det tager altså 0,1 time at løbe en kilometer. Løbere vil tit gerne vide, hvor mange *minutter*, ikke timer, det tager at løbe en kilometer. Og eftersom $1\ time = 60\ minutter$ kan vi bare stille omregningen sådan op:

$$0,1 \frac{t}{km} \cdot 60 \frac{min}{t} = 0,1 \cdot 60 \cdot \frac{t\cdot min}{km \cdot t} = 6 \frac{min}{km}$$
Det tager altså løberen 6 minutter ($min$) at løbe én kilometer. Igen: Læg mærke til at vi fandt den nye enhed $\frac{min}{km}$ ved bare at behandle enhederne som almindelige tal eller variabler.
...
Det tager altså 0,006 minutter at løbe én meter. Og hvis vil gerne vil have det i sekunder per meter, bruger vi, at der 60 sekunder ($s$) per minut: 

$$0,006 \frac{min}{m} \cdot 60 \frac{s}{min} = 0,36 \frac{s}{m}$$
...
Enheden for hertz er nemlig $s^{-1}$ og $\frac{1}{s} = s^{-1}$. Det kan godt virke forvirrende, fordi den enhed ikke er så velkendt fra hverdagen. Grunden til at vi ender med en lidt sjov enhed er, at antallet af hjerteslag bare er almindelige tælletal. Tælleren i brøken $\frac{3}{2 s}$ har med andre ord ikke nogen enhed.
